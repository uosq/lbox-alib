...
callbacks.Register("CreateMove", function(usercmd)
   ...
   if input.IsButtonDown(E_ButtonCode.MOUSE_LEFT) and verticalslider.is_mouse_inside then
      local value = alib.math.GetNewVerticalSliderValue(window, verticalslider, verticalslider.min, verticalslider.max,
         verticalslider.flipped)
   ...
   --[[ its not really useful in this for now, but at least explains how isMouseInsideList works xd
...
callbacks.Register("Draw", function()
   ...
   --- button
   alib.objects.button(button.is_mouse_inside, button.width, button.height, button.x + window.x, button.y + window.y,
      button.text)

   --- button fade horizontal
   alib.objects.buttonfade(fade_button.is_mouse_inside, fade_button.width, fade_button.height, fade_button.x + window.x,
      fade_button.y + window.y, 255, 50, true, fade_button.text)

   --- button fade vertical
   alib.objects.buttonfade(fade_button2.is_mouse_inside, fade_button2.width, fade_button2.height,
      fade_button2.x + window.x, fade_button2.y + window.y, 255, 50, false, fade_button2.text)
   ...
   --- faded slider
   alib.objects.sliderfade(fade_slider.width, fade_slider.height, fade_slider.x + window.x, fade_slider.y + window.y, 0,
      100, fade_slider.value, 50, 255, true)

   --- checked checkbox
   alib.objects.checkbox(checked_checkbox.width, checked_checkbox.height, checked_checkbox.x + window.x,
      checked_checkbox.y + window.y, checked_checkbox.checked)

   --- list
   alib.objects.list(list.width, list.x + window.x, list.y + window.y, list.selected_item_index, list.items)

   --- vertical slider
   alib.objects.verticalslider(verticalslider.width, verticalslider.height, verticalslider.x + window.x,
